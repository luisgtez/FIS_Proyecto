...
class DeportistaModel:
    ...
    def insertActivity(self,fecha,duracion_horas,localizacion,distancia_kms,FC_max,FC_min,tipo_actividad_id,subtipo_actividad_id,idDeportista, publica = False):
        ...
        query = """ 
                insert into Actividad(ID, Fecha, DuracionHoras, Localizacion, DistanciaKms, FCMax, FCMin, TipoActividadID,SubtipoActividadID, DeportistaID, Publico) values (null,?,?,?,?,?,?,?,?,?,?) 
                """ 
        # Ejecutamos la query con los parámetros correspondientes
        self.db.executeUpdateQuery(query,fecha,duracion_horas,localizacion,distancia_kms,FC_max,FC_min,tipo_actividad_id,subtipo_actividad_id,idDeportista, publica)
